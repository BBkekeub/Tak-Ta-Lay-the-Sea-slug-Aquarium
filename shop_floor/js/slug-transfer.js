...
(()=>{
 ...
 function draw(){
  const sides=[tank,other],current=q('[data-values]').value==='current';
  const scroll=[...d.querySelectorAll('.slug-list')].map(el=>el.scrollTop);
  q('[data-lists]').innerHTML=sides.map((o,i)=>{const all=o?o.slugs:G.inv,visible=all.filter(s=>matchesSlugStats(s,filters,current));selected[i]=new Set([...selected[i]].filter(id=>visible.some(s=>s.id===id&&!slugTransferLocked(s,o))));
  ...
  d.querySelectorAll('[data-all]').forEach(el=>el.onclick=()=>{const i=+el.dataset.all,o=sides[i];selected[i]=new Set((o?o.slugs:G.inv).filter(s=>matchesSlugStats(s,filters,current)&&!slugTransferLocked(s,o)).map(s=>s.id));draw();});
