...
function renderBreedingPicker(){
 ...
 breedingPicker.innerHTML='<div style="display:flex;justify-content:space-between;align-items:center"><h3 style="margin:0">'+(ready?'ผสมพันธุ์ 2 ตัวนี้เลยไหม?':'เลือกทากมาผสมพันธุ์ · '+ids.length+'/2')+'</h3><button class="tbtn" data-close>ปิด ✕</button></div><div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(135px,1fr));gap:8px;margin:16px 0;max-height:48vh;overflow:auto">'+o.slugs.map(s=>'<button class="tbtn" data-parent-id="'+s.id+'" aria-pressed="'+ids.includes(s.id)+'" style="padding:10px;border-color:'+(ids.includes(s.id)?'#f1cc75':'#50696a')+';background:'+(ids.includes(s.id)?'#465044':'#203338')+'" '+(!canChooseParent(s)||ready&&!ids.includes(s.id)?'disabled':'')+'><canvas width="180" height="132" data-slug="'+s.id+'" style="width:100%;height:75px;object-fit:contain"></canvas>'+s.id+(ids.includes(s.id)?' ✓':'')+'<br>พลังผสม '+(s.breedLife??50)+(!canChooseParent(s)?'<br>ยังเลือกไม่ได้':'')+'</button>').join('')+'</div>'+(o.slugs.length?'':'<p>ยังไม่มีทากในโซนเลี้ยง</p>')+'<div style="display:flex;gap:8px"><button class="tbtn" data-confirm '+(!ready?'disabled':'')+'>เริ่มผสม · 1 นาที</button><button class="tbtn" data-cancel>ยกเลิกการเลือก</button><button class="tbtn" data-manage>นำทากเข้าตู้</button></div>';
 breedingPicker.querySelectorAll('[data-slug]').forEach(c=>{const s=o.slugs.find(s=>s.id===c.dataset.slug);drawSlugPortrait(c,s);});
