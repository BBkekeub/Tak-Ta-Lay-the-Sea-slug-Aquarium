...
cv.addEventListener('wheel', e=>{
  ...
  cam.zoom=Math.max(0.3, Math.min(3, cam.zoom*(e.deltaY<0?1.12:1/1.12)));
...
function fitCamera(){
  ...
  cam.zoom=Math.max(0.3, Math.min(1.6, Math.min(CW/(wSpan*1.15), CH/(hSpan*1.25))));
}
...
document.getElementById('zOut').onclick=()=>{ cam.zoom=Math.max(0.3,cam.zoom/1.2); };
