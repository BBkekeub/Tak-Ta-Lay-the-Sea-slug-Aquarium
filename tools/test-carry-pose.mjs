...
const out=path.join(root,'tools/qa/carry-pose');fs.mkdirSync(out,{recursive:true});
...
const previous=execFileSync('git',['-c',`safe.directory=${root.replaceAll('\\','/')}`,'show','HEAD:js/people.js'],{cwd:root,encoding:'utf8'});
...
try{
 ...
 await page.evaluate(({before,after,rawBefore})=>{
  ...
  window.carryGallery=(draw,body)=>{
   ...
   for(let i=0;i<4;i++){
    const p=carryPerson(body,Math.PI/4+i*Math.PI/2),sx=300+(i%2)*600,sy=440+Math.floor(i/2)*490;
 ...
 report.cost=await page.evaluate(()=>{
  ...
  for(const [name,draw] of [['before',carryBeforeRaw],['after',drawPerson]]){
   ...
   for(let i=0;i<25;i++){p.phase=i*.2;p.idle=i*.1;p._drawPose=null;_personBatch.length=0;const t=performance.now();draw(p);if(i>=5)samples.push(performance.now()-t);}
   const triangles=p._drawPose.faces.reduce((n,f)=>n+f.v.length-2,0);samples.sort((a,b)=>a-b);
   costs[name]={poseBuildMedianMs:samples[Math.floor(samples.length/2)],triangles};
  }
