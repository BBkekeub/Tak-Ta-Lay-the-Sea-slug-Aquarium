...
const out=path.join(root,'tools/qa/hat-fit');fs.mkdirSync(out,{recursive:true});
...
try{
 ...
 await page.evaluate(body=>carryGallery(carryAfter,body,{accessory:'hat',carryTank:false,headYaw:.3,build:1.12}),body);
 await page.locator('#carryQA').screenshot({path:path.join(out,body+'.png')});
 }

 report.cache=await page.evaluate(()=>{
  _personBatch=[];cam.x=0;cam.y=-100;cam.zoom=.5;
  const p=carryPerson('male',.7,{accessory:'hat',carryTank:false});drawPerson(p);const pose=p._drawPose;
  cam.x+=10;cam.zoom*=1.1;drawPerson(p);const reused=p._drawPose===pose;
  p.x=100000;drawPerson(p);const offscreen=p._drawPose===pose;_personBatch=null;
  return {reused,offscreen,fitReused:Object.values(PEOPLE_MODEL).every(m=>fittedPersonHat(m)===fittedPersonHat(m))};
 });
 assert.ok(report.cache.reused&&report.cache.offscreen&&report.cache.fitReused);
 assert.deepEqual(errors,[]);report.passed=true;
}catch(e){report.failure=String(e);process.exitCode=1;}
finally{fs.writeFileSync(path.join(out,'report.json'),JSON.stringify(report,null,2));console.log(JSON.stringify({passed:report.passed,failure:report.failure,errors}));await browser?.close();server.close();}
