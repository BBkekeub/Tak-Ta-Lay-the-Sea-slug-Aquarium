...
// โปรไฟล์ใหม่ทุกครั้ง ไม่แตะเซฟของผู้เล่น · ภาพออกที่ tools/qa/throw-real/
...
try{
 ...
 const page=await browser.newPage({viewport:{width:1280,height:800}});page.on('pageerror',e=>errors.push(e.message));page.on('requestfailed',r=>report.network.push({url:r.url(),failure:r.failure()?.errorText}));
 await page.route('**/js/slug-throw.js*',async route=>{let s=fs.readFileSync(path.join(root,'js/slug-throw.js'),'utf8');s=s.replace('window.SlugThrow={','window.SlugThrow={_qa:{camera,waveImage,stoneImage,loadedStone,updateHud,getSprites:()=>sprites},');await route.fulfill({contentType:'text/javascript',body:s});});
 ...
 for(const angle of [0,45,90]){
   await page.evaluate(angle=>{const a=G.throwing.active;a.phase='power';a.lockAngle=angle;a.lockPower=.8;drawTankFrame();},angle);
   await page.waitForTimeout(200);await page.screenshot({path:path.join(out,'pose-'+angle+'.png')});
 }
 report.assets=await page.evaluate(()=>({stoneReused:SlugThrow._qa.stoneImage()===SlugThrow._qa.stoneImage(),laneCm:SlugThrow.LANE_H*CM_PER_CELL}));
 assert.equal(report.assets.laneCm,20);assert.ok(report.assets.stoneReused);
 ...
 report.waveAxis=await page.evaluate(()=>{
  const a=G.throwing.active;a.phase='waves';a.clock=.8;
  a.shot.directionX=1;a.shot.directionZ=-1;a.shot.pitch=20;
  const image=SlugThrow._qa.waveImage(),original=tctx.drawImage,transforms=[];
  tctx.drawImage=function(bitmap,...args){if(bitmap===image){const m=this.getTransform();transforms.push({b:m.b,c:m.c});}return original.call(this,bitmap,...args);};
  try{SlugThrow.drawItem({part:'stone'});}finally{tctx.drawImage=original;}
  return transforms;
 });
 assert.ok(report.waveAxis.length>0);assert.ok(report.waveAxis.every(m=>Math.abs(m.b)<1e-8&&Math.abs(m.c)<1e-8),'Wave stays on X while stone tilts');
