...
(()=>{
 ...
 function draw(){
  /* แผงกรองอยู่ในกล่องของตัวเอง "ใต้หัวเรื่อง" — เดิม mount() prepend ลงหน้าต่างเลย
     แผงกรองเลยไปอยู่เหนือบรรทัด "จัดการทาก" กับปุ่มปิด (ผู้เล่นทัก 2026-09-18) */
  SlugBrowser.mount(q('[data-filterhost]'),'transfer',draw);
  ...
  d.querySelectorAll('[data-preview]').forEach(c=>{const o=sides[+c.dataset.side],s=(o?o.slugs:G.inv).find(s=>s.id===c.dataset.preview);drawSlugPortrait(c,s);SlugHover.mark(c.parentElement,s);SlugBrowser.heart(c.parentElement,s,draw,'transfer');});
