...
function drawWallShelf(preview) {
  ...
  for (let tier = 0; tier < SHELF_TIERS; tier++) {
    ...
    /* ฉากยึดผนัง 2 ตัว — ต้องวาด "ก่อน" แผ่นชั้นเสมอ
       มันอยู่ชิดผนัง (ลึกน้อย) และต่ำกว่าแผ่น = อยู่ไกลกล้องกว่า ตามกฎช่างวาดจึงต้องมาก่อน
       ของเดิมวาดทีหลัง เงาของมันเลยไปทับบนหน้าแผ่นชั้น เห็นเป็นลูกบาศก์ทะลุขึ้นมาบนแผ่น
       (ตำแหน่ง/ขนาดถูกอยู่แล้ว ปัญหาอยู่ที่ลำดับล้วน ๆ)
       ขนาด: ชิดผนังลึก 5 ซม. · หย่อนไม่เกิน (SHELF_D - ลึกฉาก)*TH ไม่งั้นโผล่ต่ำกว่าขอบหน้าแผ่น */
