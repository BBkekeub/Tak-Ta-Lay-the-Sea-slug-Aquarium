...
SlugHover.attach(showcaseDialog);   // ชี้/กดค้างการ์ดทาก = แผงยีน เหมือนหน้าเลือกทากผสมพันธุ์ (slug-hover.js)
...
function renderComputer(){
 ...
 for(const m of (list?computerInbox():[])){const card=document.createElement('details'),title=document.createElement('summary'),body=document.createElement('p');card.style.cssText='padding:14px;margin:8px 0;border:1px solid #526663;border-radius:8px';title.textContent=(m.read?'':'!! · ')+(m.type==='quest'?'เควส · ':'ออนไลน์ · ')+m.title;body.textContent=m.body;body.style.whiteSpace="pre-line";card.append(title,body);if(m.id===FIVE_MIN_GIFT){const _got=!!m.claimed||!!G.claimed?.[FIVE_MIN_GIFT];const claim=document.createElement('button');claim.className='tbtn';claim.dataset.claimGift='';claim.textContent=_got?'รับเงินแล้ว ✓':'รับเงินแนบ 500 เหรียญ';claim.disabled=_got;claim.onclick=()=>{if(claimFiveMinuteGift()){claim.disabled=true;claim.textContent='รับเงินแล้ว ✓';}};card.append(claim);}if(typeof decorateInboxCard==='function'){try{decorateInboxCard(card,m);}catch(_e){console.warn('[inbox] การ์ดเสริมพัง',_e);}}const _del=document.createElement('button');_del.className='tbtn';_del.textContent='🗑 ลบจดหมาย';_del.style.cssText='margin-top:8px;display:block';_del.onclick=(ev)=>{ev.preventDefault();ev.stopPropagation();const _a=computerInbox(),_i=_a.indexOf(m);if(_i>=0)_a.splice(_i,1);if(typeof saveGame==='function')saveGame();renderComputer();};card.append(_del);card.ontoggle=()=>{if(card.open&&!m.read){m.read=true;title.textContent=(m.type==='quest'?'เควส · ':'ออนไลน์ · ')+m.title;syncTabBadges();saveGame();}};list.append(card);}
 computerDialog.querySelector('[data-close]').onclick=()=>computerDialog.close();
 /* ⚠️ เดิมหาปุ่มจาก "ข้อความขึ้นต้นว่า ข้อเสนอ" ใน .context-nav — ผังใหม่ 2026-09-13 ย้ายปุ่มนี้ไปถาดแจ้งเตือนขวาล่าง
    หาไม่เจอแล้ว ปุ่มในคอมพิวเตอร์ร้านจะกดแล้วเงียบ · ยึด id แทน (context-ui.js ตั้ง #navOffers ให้ตายตัว) */
...
function claimFiveMinuteGift(){
 ...
 mail.claimed=true;mail.read=true;addCoin(500);syncHUD();saveGame();toast('รับเงินจากผู้หวังดี +500 เหรียญ','good');return true;
}
