...
function renderComputer(){
 computerDialog.innerHTML='<div style="display:flex;align-items:center;justify-content:space-between"><b>คอมพิวเตอร์ร้าน</b><button class="tbtn" data-close aria-label="ปิด">✕</button></div><button class="tbtn" data-offers style="margin:16px 8px 0 0">ข้อเสนอหน้าร้าน ('+TRADE_OFFERS.length+')</button><button class="tbtn" data-orderbox style="margin:16px 0 0">🛒 สั่งซื้อกล่องทาก'+(typeof slugDeliveryReadyCount==='function'&&slugDeliveryReadyCount()?' · 📦'+slugDeliveryReadyCount():'')+'</button><h3>เควส / ออเดอร์ออนไลน์</h3><div data-inbox></div>';
 const list=computerDialog.querySelector('[data-inbox]');if(!computerInbox().length)list.textContent='ยังไม่มีรายการใหม่';
 for(const m of computerInbox()){const card=document.createElement('details'),title=document.createElement('summary'),body=document.createElement('p');card.style.cssText='padding:14px;margin:8px 0;border:1px solid #526663;border-radius:8px';title.textContent=(m.read?'':'!! · ')+(m.type==='quest'?'เควส · ':'ออนไลน์ · ')+m.title;body.textContent=m.body;body.style.whiteSpace="pre-line";card.append(title,body);if(m.id==='five-minute-gift'){const claim=document.createElement('button');claim.className='tbtn';claim.dataset.claimGift='';claim.textContent=m.claimed?'รับเงินแล้ว ✓':'รับเงินแนบ 500 เหรียญ';claim.disabled=!!m.claimed;claim.onclick=()=>{if(claimFiveMinuteGift()){claim.disabled=true;claim.textContent='รับเงินแล้ว ✓';}};card.append(claim);}card.ontoggle=()=>{if(card.open&&!m.read){m.read=true;title.textContent=(m.type==='quest'?'เควส · ':'ออนไลน์ · ')+m.title;saveGame();}};list.append(card);}
